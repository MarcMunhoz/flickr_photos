<template>
  <div
    ...
    :class="{ 'is-empty': photos.length === 0 && !error && !isLoading }"
  >
    <p v-if="error" class="text-danger w-100">{{ error }}</p>

    <div v-if="isLoading" class="spinner text-info w-100">
      <div class="spinner-border" role="status" />
      <p class="fw-bold">Loading... Please wait.</p>
    </div>

    <div v-show="!isLoading" class="gallery d-flex flex-row flex-wrap justify-content-around mx-auto">
      <section v-if="photos.length === 0 && !error && !hasSearched" class="home-empty-state">
      ...
      <div class="w-100" v-if="photos.length > 0">
        <h1>
          Photos by <span class="text-success fw-bolder">{{ ownerName }}</span> on Flickr - Page {{ currentPage }} of {{ totalPages }}
        </h1>
      ...
      <button v-if="hasPreviousPage && photos.length > 0" @click="goToPage(currentPage - 1)" class="btn btn-primary simple-font fw-bold">Page [ {{ currentPage - 1 }} ]</button>
      <button v-if="hasNextPage && photos.length > 0" @click="goToPage(currentPage + 1)" class="btn btn-primary simple-font fw-bold">Page [ {{ currentPage + 1 }} ]</button>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    onUnmounted(() => {
      gallery.cancelLoading();
    });

    return {
      theDate: formatPhotoDate,
      ...gallery,
    };
  },
});
